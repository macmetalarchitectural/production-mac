...
class WebCustomLabel(models.Model):
  ...
  lang = fields.Selection(_lang_get, 'Language', default=lambda self: self.env.lang, required=True)
  model_ids = fields.Many2many('ir.model', 'ir_model_custom_label', 'label_id', 'model_id', 'Model')
  type_ = fields.Selection([('field', 'Field'), ('xpath', 'Xpath'),], default='field', required=True)
  ...
  def get(self, model, lang):
    ...
    """
    return self.sudo().env['web.custom.label'].search([('model_ids.model', '=', model), ('lang', '=', lang)]).read()
