...
class ResPartner(models.Model):
    ...
    def write(self, vals):
        if vals.get("customer_state") == "approved" and self.filtered(
            lambda p: p.is_company or not p.parent_id
        ):
    ...
    def _check_customer_restricted_fields(self, vals):
        if not self.env.user.has_group("partner_validation_sale.group_partner_restricted_field_sales"):
            restricted_fields = (
                ...
                .sudo()
                .search([("apply_on_sales", "=", True)], limit=None)
            ...
            if final_restricted_fields and restricted_customers:
                raise ValidationError(
                    _(
                        "You are not authorized to modify the following fields: "
                        "%s when the partner's status is approved."
                    )
